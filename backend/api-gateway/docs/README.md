    ...
    *   Пример ответа:
      ```json
      {
        ...
        "dependencies": {
          ...
          "catalog_service": "degraded",
          "payment_service": "healthy"
        },
        "version": "1.0.0"
      }
      ```
    *   *Примечание: Актуальный список зависимостей и механизм проверки их статуса будут реализованы в процессе разработки и могут динамически обновляться.*
*   **`GET /api/docs`**
...
*   Конфигурация gRPC проксирования будет зависеть от выбранного ПО для API Gateway (например, Kong, Tyk, или Envoy-based). Как правило, это включает определение gRPC маршрутов, указание backend gRPC сервисов (через Kubernetes services), и настройку TLS для безопасного соединения. Для Kong, например, это может включать создание `Service` с `protocol: grpc` и `Route` с соответствующими `paths` или `hosts`. Для gRPC-Web трансляции могут потребоваться дополнительные фильтры или плагины. Детальная конфигурация будет приведена в операционной документации шлюза.
...
*   Основные переменные окружения (зависят от выбранного ПО для API Gateway, например, Kong, Tyk):
    *   `GW_LOG_LEVEL`: Уровень логирования (например, `info`, `debug`, `error`).
    *   `GW_LISTEN_PORT_HTTP`: Порт для HTTP трафика (например, `8080`).
    *   `GW_LISTEN_PORT_HTTPS`: Порт для HTTPS трафика (например, `8443`).
    *   `GW_ADMIN_API_PORT`: Порт для административного API шлюза (например, `8001` для Kong).
    *   `GW_KUBERNETES_NAMESPACE`: Пространство имен Kubernetes для обнаружения сервисов.
    *   `GW_KUBERNETES_INGRESS_CLASS`: Класс Ingress, если используется интеграция.
    *   `GW_AUTH_SERVICE_URL`: URL или имя сервиса Auth Service для валидации токенов.
    *   `GW_CORS_ALLOWED_ORIGINS`: Список разрешенных источников для CORS (например, `"http://localhost:3000,https://gameplatform.ru"`).
    *   `GW_RATE_LIMITING_REDIS_URL`: (Если используется Redis для rate limiting) URL для подключения к Redis.
    *   `KONG_DATABASE`: (Для Kong) Тип базы данных (`postgres`, `off`).
    *   `KONG_PG_HOST`, `KONG_PG_USER`, `KONG_PG_PASSWORD`, `KONG_PG_DATABASE`: (Для Kong с PostgreSQL).
    *   `KONG_PLUGINS`: Список подключаемых плагинов Kong (например, `jwt,cors,rate-limiting`).
    *   `TYK_GW_STORAGE_TYPE`, `TYK_GW_STORAGE_HOST`, `TYK_GW_STORAGE_PORT`, `TYK_GW_STORAGE_PASSWORD`: (Для Tyk с Redis).
    *   `TYK_GW_MIDDLEWARE_GLOBAL`: Глобальные middleware для Tyk.
*   *Примечание: Конкретный набор переменных окружения будет зависеть от выбранного программного обеспечения API Gateway и конфигурации его развертывания. Приведенные примеры являются общими.*
