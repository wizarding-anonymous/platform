...
<!-- Важно: При обновлении версий в project_technology_stack.md, не забудьте синхронизировать их здесь! -->
...
*   Перейдите в директорию клиента:
    ```bash
    cd client/flutter_app/ # Или актуальный путь к клиенту
    ```
*   Сборка для Android:
    ```bash
...
**1. Запуск тестов для Go-сервисов:**
...
**2. Запуск тестов для Flutter-клиента:**
*   Перейдите в директорию клиента:
    ```bash
    cd client/flutter_app/ # Или актуальный путь к клиенту
    ```
...
**3. Использование Makefile (если предусмотрено):**
*   Проверьте наличие `Makefile` в корне проекта или в директориях сервисов.
*   Часто используемые команды:
    ```bash
