...
## 8. Стандарты Инфраструктурных Файлов

### 8.1. Dockerfile
*   Многоэтапные сборки. Официальные базовые образы. Минимизация размера. Non-root пользователь. Тегированные версии.
*   (Пример Dockerfile для Go см. в исходном документе "Стандарты API...")

### 8.2. Docker Compose (для разработки)
*   Версия 3.x. Группировка сервисов. Сети, тома. Переменные из `.env`.
*   (Пример docker-compose.yml см. в исходном документе "Стандарты API...")

### 8.3. Kubernetes
*   Helm для управления манифестами. ConfigMaps/Secrets. Resource requests/limits. Liveness/Readiness probes.
*   (Примеры Deployment, Service, ConfigMap, Secret см. в исходном документе "Стандарты API...")

### 8.4. CI/CD
*   Примеры для GitHub Actions и GitLab CI/CD, включающие этапы test, build, deploy.
*   (Примеры см. в исходном документе "Стандарты API...")
