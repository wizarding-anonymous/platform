...
*   **Локализация Персональных Данных:** В соответствии с ФЗ-152, персональные данные российских пользователей, собранные `Auth Service` (например, email, телефон) и `Account Service` (например, ФИО, никнейм, если являются ПДн), хранятся и обрабатываются в базах данных, расположенных на территории РФ. API Gateway обеспечивает маршрутизацию запросов к экземплярам этих сервисов, развернутым в российских дата-центрах.
*   **Трансграничная передача данных:** Если для каких-либо функций (например, аналитика, внешние уведомления) предполагается передача обезличенных или агрегированных данных за пределы РФ, это должно быть явно описано и соответствовать законодательству РФ о персональных данных.
